...
export class UserController {
  ...
  async loggedInUser(@Req() req: Request) {
    ...
    const carmaAndNumberOfQuotes =
      await this.userService.carmaAndNumberOfQuotes(user.id);
    return {
      user,
      carma: carmaAndNumberOfQuotes.carma,
      quotes: carmaAndNumberOfQuotes.numOfQuotes,
    };
  }
  ...
  async getUserWithId(@Param('id') id: string) {
    ...
    user.password = undefined;
    const carmaAndNumberOfQuotes =
      await this.userService.carmaAndNumberOfQuotes(id);
    return {
      user,
      carma: carmaAndNumberOfQuotes.carma,
      quotes: carmaAndNumberOfQuotes.numOfQuotes,
  ...
  @ApiCreatedResponse({
    description: 'Updates the password of signed in the user.',
  })
  ...
  @ApiCreatedResponse({
    description:
      'Updates the email, first name, or last name of the signed in user.',
  })
  @Patch('update')
